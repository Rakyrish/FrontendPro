...
app.post('/api/exams', async (req, res) => {
  const { teacher_name, subject, exam_date } = req.body;
  try {
    const newExam = await insertAndReturn(
      `INSERT INTO Exams (teacher_name, subject, exam_date) VALUES (?, ?, ?)`,
      [teacher_name, subject,exam_date],
      'Exams',
      'exam_id'
    );
    res.json(newExam);
...
app.post('/api/exam_results', async (req, res) => {
  const {student_name, subject, marks_obtained } = req.body;
  try {
    const newExamResult = await insertAndReturn(
      `INSERT INTO Exam_Results ( student_name, subject, marks_obtained) VALUES (?, ?, ?)`,
...
app.post('/api/attendance', async (req, res) => {
  ...
  try {
    const newAttendance = await insertAndReturn(
      `INSERT INTO Attendance (student_name, class_name, date, status) VALUES (?, ?, ?, ?)`,
...
app.post('/api/fees_payments', async (req, res) => {
  const { amount_paid, payment_date, student_name } = req.body;
  try {
    const newPayment = await insertAndReturn(
      `INSERT INTO Fees_Payments (amount_paid, payment_date,student_name) VALUES (?, ?,?)`,
      [amount_paid, payment_date, student_name],
      'Fees_Payments',
      'payment_id'
    );
    res.json(newPayment);
